...
namespace HtmlAgilityPackCore
{
    public abstract class CssSelector
    {
        ...
        {
            return selector.AsSpan().Slice(Token.Length).ToString();
        }
        ...
        {
            var selector = char.IsLetter(token.Filter.Span[0])
                ? Selectors.First(i => i is TagNameSelector)
                : Selectors.Where(s => s.Token.Length > 0).FirstOrDefault(s => token.Filter.ToString().StartsWith(s.Token));

            if (selector == null)
                throw new InvalidOperationException($"Invalid token : {token.Filter.ToString()}.");

            var selectorType = selector.GetType();
            var rt = (CssSelector)Activator.CreateInstance(selectorType);

            string filter = token.Filter.Slice(selector.Token.Length).ToString();
